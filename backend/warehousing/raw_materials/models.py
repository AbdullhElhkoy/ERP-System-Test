...
class MaterialStorage(models.Model):
    plant = models.ForeignKey("plants.Plant", on_delete=models.PROTECT, related_name="material_storages")
    material = models.ForeignKey(Material, on_delete=models.PROTECT, related_name="storages")
    storage_code = models.CharField(max_length=30, unique=True)
    storage_name = models.CharField(max_length=100)
    description = models.TextField(blank=True)
    allow_estimated_issue = models.BooleanField(default=False)
    is_active = models.BooleanField(default=True)
    ...
    def current_balance(self):
        totals = self.transactions.aggregate(
            incoming=Sum(
                Case(When(movement_type=InventoryTransaction.MOVEMENT_IN, then="quantity_tons"),
                     default=Decimal("0"), output_field=DecimalField())
            ),
            outgoing=Sum(
                Case(When(movement_type=InventoryTransaction.MOVEMENT_OUT, then="quantity_tons"),
                     default=Decimal("0"), output_field=DecimalField())
            ),
            adjustments=Sum(
                Case(When(movement_type=InventoryTransaction.MOVEMENT_ADJUSTMENT, then="quantity_tons"),
                     default=Decimal("0"), output_field=DecimalField())
            ),
        )
        return (
    ...
    def __str__(self):
        return f"{self.plant.plant_name} - {self.material.material_name} - {self.storage_name}"
...
class RawMaterialDelivery(models.Model):
    ...
    notes                = models.TextField(blank=True)
    user                 = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="raw_material_deliveries")
...
class InventoryTransaction(models.Model):
    ...
    reference_delivery = models.ForeignKey(
        RawMaterialDelivery, null=True, blank=True,
...
class RawMaterialSample(models.Model):
    ...
    delivery = models.ForeignKey(
        RawMaterialDelivery, on_delete=models.CASCADE,
    ...
    user          = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="raw_material_samples")
    notes         = models.TextField(blank=True)
...
class RawMaterialAnalysis(models.Model):
    ...
    def save(self, *args, **kwargs):
        specification = (
            ...
            .filter(material=self.sample.material, test=self.test, is_active=True)
            .first()
        )
        if specification and self.result is not None:
            ok = True
            if specification.specification_min is not None and self.result < specification.specification_min:
                ok = False
            if specification.specification_max is not None and self.result > specification.specification_max:
                ok = False
